...
def _coerce_version(raw_version: Any) -> list[int]:
    """Some third-party addons don't follow the [major, minor, patch] array
    format exactly (e.g. version as the string "1.11.0", or an array of the
    wrong length). Parse leniently instead of rejecting the manifest."""
    if isinstance(raw_version, str):
        parts: list[Any] = re.split(r"[.,]", raw_version.strip())
...
def pack_identity(manifest: dict[str, Any]) -> tuple[str, list[int]]:
    ...
    version = _coerce_version(header.get("version", [1, 0, 0]))
    return str(header["uuid"]), version
