...
MAX_URL_DOWNLOAD_BYTES = int(os.environ.get("ADDON_MAX_DOWNLOAD_MB", "300")) * 1_000_000
...
def unpack_addon_archive(archive_path: Path, output_dir: Path) -> list[Path]:
    ...
    while pending:
        ...
        for nested in destination.rglob("*"):
            if nested.is_file() and nested.suffix.lower() in {".mcpack", ".zip"}:
...
def install_addon_archive(
    ...
    with tempfile.TemporaryDirectory(prefix="minecraft-addon-") as temporary_dir:
        ...
        for pack_dir in pack_dirs:
            ...
            header = manifest.get("header", {})
            raw_name = str(header.get("name") or pack_id)
            folder_name = re.sub(r"[^A-Za-z0-9_.-]+", "_", raw_name).strip("._")
            folder_name = folder_name[:80] or pack_id
            update_status(
            ...
            installed.append(f"{kind}: {raw_name}")
...
def download_archive_from_url(url: str, update_status: Callable[[str], None]) -> Path:
    """Download an addon archive from a direct link (or a Google Drive share link)."""
...
def run_ai_task(
    ...
    try:
        ...
        try:
            available_model_ids = [
                str(model.id) for model in client.models.list().data if getattr(model, "id", None)
            ]
...
def main() -> None:
    ...
    def handle_start(message: telebot.types.Message) -> None:
        ...
        bot.reply_to(
            ...
            "(в том числе Google Drive) — размер файла ограничен только "
            "лимитом сервера.",
    ...
    def handle_message(message: telebot.types.Message) -> None:
        nonlocal busy, awaiting_sftp_settings
        if not message.from_user or message.from_user.id != allowed_user_id:
            bot.reply_to(message, "Доступ ограничен")
            return
        ...
        if awaiting_sftp_settings:
            ...
            try:
                ...
                return
            save_json_file(RUNTIME_CONFIG_FILE, {"host": host, "port": port})
            awaiting_sftp_settings = False
            bot.reply_to(
                message,
